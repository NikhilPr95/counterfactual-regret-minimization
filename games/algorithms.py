    ...
    def _cfr_utility_recursive(self, state, reach_a, reach_b):
        ...
        if state.is_chance():
            if self.chance_sampling:
                ...
                return state.chance_prob() * sum([self._cfr_utility_recursive(outcome, reach_a, reach_b)
                                                  for outcome in chance_outcomes])

        # sum up all utilities for playing actions in our game state
        value = 0.
        for action in state.actions:
            child_reach_a = reach_a * (self.sigma[state.inf_set()][action] if state.current_player.team == 1 else 1)
            child_reach_b = reach_b * (self.sigma[state.inf_set()][action] if state.current_player.team == 2 else 1)
        ...
        (cfr_reach, reach) = (reach_b, reach_a) if state.current_player.team == 1 else (reach_a, reach_b)
